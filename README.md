...
By default, we use hotpotqa and triviaqa as our training data which has been put in directory `Search-R1/scripts/data_process/data`. The training and testing datasets are generated by scripts `qa_search_train_merge.py` and `qa_search_test_merge.py` under directory `Search-R1/scripts/data_process`.
...
We provide the final training data for the first iteration training in `FlashRAG/examples/methods/training_data/retriever_training_data.jsonl` .
